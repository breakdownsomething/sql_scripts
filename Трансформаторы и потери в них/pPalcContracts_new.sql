--if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[pCalcContract]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
--drop procedure [dbo].[pCalcContract]
GO
...
SELECT
  @tiServId=CONVERT(TinyInt,MIN(SERV_ID))
 FROM
  ServiceTypes
 WHERE
  SERV_NAME='Электроэнергия'
  ...
  @tiActivPDecodeId=
   (SELECT
     ...
     FROM
      ProDecode (NoLock)
     WHERE
      DECODE_NAME='За активную энергию')
Print '*Доп.сумма='
Print @tiAddDecodeId
...
SELECT
  @tiCountMethodId=
   (SELECT
      MAX(AUDIT_METHOD_ID)
     FROM
      ProAuditMethods (NoLock)
     WHERE
      AUDIT_METHOD_NAME='Учет по счетчику'),
  @tiPartMethodId=
   (SELECT
      MAX(AUDIT_METHOD_ID)
     FROM
      ProAuditMethods (NoLock)
     WHERE
      AUDIT_METHOD_NAME='Долевой учет')
...
SELECT
  @tiActivePowerId=
   (SELECT
      MAX(POWER_GROUP_ID)
     FROM
      ProPowerGroups (NoLock)
     WHERE
      POWER_GROUP_NAME='Активная'),
  @tiReActivePowerId=
   (SELECT
      MAX(POWER_GROUP_ID)
     FROM
...
IF Exists
  (SELECT *
    ...
    WHERE id = OBJECT_ID('TempDB..#TmpTVal')
  )
  DROP TABLE
    #TmpTval
...
IF Exists
  (SELECT *
    FROM TempDB..SysObjects
    WHERE id = OBJECT_ID('TempDB..#TmpPnt')
  )
  DROP TABLE
    #TmpPnt
...
IF Exists
  (SELECT *
    FROM TempDB..SysObjects
    WHERE id = OBJECT_ID('TempDB..#TmpPQua')
  )
  DROP TABLE
    #TmpPQua
...
INSERT
 ...
 SELECT
   A.ACCOUNT_ID,
   QUANTITY=IsNull(ROUND(
       (ISNULL(CC.QUANTITY,0)*@dfIncrease*
...
IF Exists
  (SELECT *
    FROM TempDB..SysObjects
    WHERE id = OBJECT_ID('TempDB..#TmpQRea')
  )
  DROP TABLE
    #TmpQRea
...
BEGIN
  ...
  SELECT
    ...
    (SELECT
      ...
      FROM
        #TmpTVal TV (NoLock)
      WHERE
        TV.TARIFF_ID=@iTariffId),
    @vcTariffGroupId=
    (SELECT
       MAX(TV.TARIFF_GROUP_ID)
      FROM
        #TmpTVal TV (NoLock)
      WHERE
        TV.TARIFF_ID=@iTariffId)
  ...
  SELECT
    @dfSumCalc=Round(ISNULL(@iPower*@dfTariffValue,0),2)
  IF @dfSumCalc<>0
    INSERT
     #TmpCalc
     (CALC_NUMBER,
  ...
  SELECT
   ...
   FROM
    ProCalcTypes (NoLock)
   WHERE
    CALC_TYPE_NAME='Надбавка за реактивную энергию'
  Print '*Надбавка за реактивную энергию='
  Print @tiCalcTypeId

  SELECT
    @iQntRea=SUM(CC.Quantity)
   FROM
    #TmpPnt A (NoLock),
    ProCntCounts CC (NoLock)
   WHERE
    A.ACCOUNT_OWNER_ID=@iAccountOwnerId AND
--    A.AUDIT_METHOD_ID=@tiCountMethodId AND
    ...
    A.AUDIT_PARAM_ID=
      (SELECT
         MAX(AUDIT_PARAM_ID)
        FROM
         ProAuditParams (NoLock)
        WHERE
         AUDIT_PARAM_NAME='Полезный расход')

  SELECT
    @iQPowRea=
      CASE
       WHEN  @siSignRea=1 THEN
        @iPowRea*3
       ELSE
        @iPowRea
      END
  ...
  SELECT
   ...
   WHERE
    ...
    (SELECT
       MAX(TT.DATE_CALC)
      FROM
       ProTariffValues TT (NoLock)
      WHERE
       TT.SERV_ID=TV.SERV_ID AND
  ...
  SELECT
    ...
    @dfSumReactive=Round(
     CASE
      WHEN ISNULL(@iQPowRea,0)>=ISNULL(@iQntRea,0) THEN
       CONVERT(Decimal(18,2),ISNULL(@iQntRea,0)*ISNULL(@dfMinTariffValue,0))
      ELSE
       CONVERT
        (Decimal(18,2),
         (
...
DECLARE curGroupAccounts CURSOR FOR
 SELECT DISTINCT
  AUDIT_GROUP_ID=CONVERT(SmallInt,ISNULL(P.KNOT_MAIN,0))
 FROM
  #TmpPnt P
 ORDER BY
  AUDIT_GROUP_ID
...
then (select LOSS_QUANTITY from ProTrancPowerLoss where account_id = @iAccountId and date_calc = @dtCalcEnd)
------------------------------------------------------------
          WHEN @tiAuditMethodId=@tiPartMethodId THEN
            @iQuantity
          WHEN @tiAuditMethodId=2 THEN
            CONVERT(Integer,ROUND(@dfCalcFactor,0)+@iAddQuantity+@iAddHCP)
          WHEN @tiAuditMethodId=3 THEN
            CONVERT(Integer,ROUND(@dfCalcFactor*@siMonthHours,0)+@iAddQuantity+@iAddHCP)
          WHEN @tiAuditMethodId=9 THEN
            CONVERT(Integer,ROUND(@dfCalcFactor*@siMonthHours,0)+@iAddQuantity+@iAddHCP)
          ELSE
            CASE
              WHEN (@siCalcStatus=1 OR @tiStatus=1) THEN
                @iPQuantity
              ELSE
                @iQuantity
            END
      ...
      IF @siAuditGroupId<>@siFirstGroupId
        SELECT
          @iSumAccount=@iSumAccount+@iQuantity*@siGroupSign
      ...
      UPDATE
       ...
       SET
        ...
        EDIT_COUNT=
          CASE
            WHEN (@siCalcStatus=1 OR @tiStatus=1) AND @tiAuditMethodId=@tiCountMethodId THEN
              Convert(Integer,
               @iEditCountBegin+
                CASE
                 WHEN @dfCalcFactor=0 THEN
                   0
                 ELSE
                   @iPQuantity/@dfCalcFactor
...
SELECT
  @iAllQuantity=IsNull(SUM
    (CASE
       WHEN CD.MEASURE_ID = @tiKBt_ChMeasureId THEN
         CD.CALC_QUANTITY
       ELSE
         0
     END),0),
  @dfSumCount=IsNull(SUM
    (CASE
       WHEN CD.MEASURE_ID NOT IN(7,8,9) THEN
         CD.SUM_CALC
       ELSE
         0
     END),0)
 FROM
  #TmpCalc CD (NoLock)
 WHERE
  CD.DECODE_ID<>0 AND CD.DECODE_ID<>@tiAddDecodeId
