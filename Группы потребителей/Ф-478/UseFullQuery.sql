...
ALTER TABLE
 ...
 SELECT
  CONSUMER_GROUP_ID    = PAG.abonent_group_id * 10,
...
INSERT
 INTO
  #TmpRez
 SELECT
  CONSUMER_GROUP_ID=Convert(SmallInt,1000),
  CONSUMER_GROUP_NAME=Convert(VarChar(30),'ИТОГО:'),
  BEG_SALDO_DB=Convert(Decimal(18,2),SUM(C.BEG_SALDO_DB)),
  BEG_SALDO_CR=Convert(Decimal(18,2),SUM(C.BEG_SALDO_CR)),
  BEG_SALDO=Convert(Decimal(18,2),SUM(C.BEG_SALDO)),
  QUANTITY=Convert(Integer,SUM(C.QUANTITY)),
...
INSERT
 INTO
  #TmpRez
 SELECT
  CONSUMER_GROUP_ID=Convert(SmallInt,25),
  CONSUMER_GROUP_NAME=Convert(VarChar(30),'Итого ромышленные'),
  BEG_SALDO_DB=Convert(Decimal(18,2),SUM(C.BEG_SALDO_DB)),
  BEG_SALDO_CR=Convert(Decimal(18,2),SUM(C.BEG_SALDO_CR)),
  BEG_SALDO=Convert(Decimal(18,2),SUM(C.BEG_SALDO)),
  QUANTITY=Convert(Integer,SUM(C.QUANTITY)),
 ...
 WHERE
  C.CONSUMER_GROUP_ID = PCG.CONSUMER_GROUP_ID and
  PAG.abonent_group_id = PCG.top_group_id and
  PAG.abonent_group_id in (1,2)
...
INSERT
 INTO
  #TmpRez
 SELECT
  CONSUMER_GROUP_ID=Convert(SmallInt,75),
  CONSUMER_GROUP_NAME=Convert(VarChar(30),'Итого бюджетные'),
  BEG_SALDO_DB=Convert(Decimal(18,2),SUM(C.BEG_SALDO_DB)),
  BEG_SALDO_CR=Convert(Decimal(18,2),SUM(C.BEG_SALDO_CR)),
  BEG_SALDO=Convert(Decimal(18,2),SUM(C.BEG_SALDO)),
  QUANTITY=Convert(Integer,SUM(C.QUANTITY)),
